...
# Custom CSS for department cards and styling
st.markdown("""
<style>
    .main-header {
        text-align: center;
...
def show_main_page():
    """Display the main landing page with department cards"""
    
    # Main header
    st.markdown("""
    <div class='main-header'>
        <h1>🏢 Shared Services Digital Catalogue</h1>
        <p>Select a department to explore available services</p>
    </div>
    """, unsafe_allow_html=True)
...
def show_department_page(department_name):
    """Display the detailed service catalogue for a selected department"""
    ...
    if dept_info.get("soon", False):
        ...
        with tab1:
            ...
            if department_name == "Information Technology":
            ...
            elif department_name == "Facility":
                df = service_data['facility_services']
                
                # Display services
                for idx, service in df.iterrows():
        ...
        with tab2:
            ...
            if department_name == "Procurement":
                df = service_data['procurement_services']
            ...
            elif department_name == "Information Technology":
        ...
        with tab3:
            ...
            with st.form("service_request_form"):
                ...
                available_services = ["Select a service..."]
                if f'{dept_key}_services' in service_data:
                    available_services.extend([s['service_name'] for s in service_data[f'{dept_key}_services'].to_dict('records')])
                
                service_requested = st.selectbox("Service Requested", available_services)
